...
- `src/checkin.py` — 출석 판정 핵심. `attempt_endfield(page) -> (status, utc8_day)`. 상태값: `success | already | login_required | unknown | error` (알림·로그 문구는 `src/app/i18n.py` 소유)
- `src/app/` — 상시 에이전트(FastAPI). `main.py`(앱·lifespan), `scheduler.py`(APScheduler), `runner.py`(Executor 직렬 실행), `settings.py`(Pydantic), `i18n.py`(문구 카탈로그), `notify.py`(httpx), `vnc.py`(WS→TCP), `templates/index.html`
...
- **`src/app/runner.py` 알림 조건:** 당일 첫 출석·상태 변화·오류만 전송. 중복 `already`는 `claimed_day == day`면 알림 없음.
