...
async def _cycle_job(reason):
    loop = asyncio.get_running_loop()
    await loop.run_in_executor(runner.executor, runner.do_cycle, reason)
    _refresh_next_run()
...
def request_cycle() -> bool:
    ...
    scheduler.add_job(_cycle_job, trigger="date", run_date=datetime.datetime.now(state.UTC8),
                      args=("수동 실행",), id="once-cycle", replace_existing=True)
    return True


def request_login(minutes=state.LOGIN_WINDOW_MINUTES) -> bool:
    scheduler.add_job(_login_job, trigger="date", run_date=datetime.datetime.now(state.UTC8),
...
def start_boot_jobs():
    ...
    if not os.path.isfile(checkin.STATE_FILE):
        ...
        scheduler.add_job(_cycle_job, trigger="date", run_date=datetime.datetime.now(state.UTC8),
                          args=("시작 시 미출석 → 즉시 실행",), id="boot-cycle", replace_existing=True)
